...
export default function DFWAccessControlGuide2026() {
  ...
  const recs: Record<string, string> = {
    contractor: '🔑 Contractor Access Management: Install a Schlage Encode Plus or Yale Assure 2 on your main entry ($180-$280). Create temporary PIN codes for each ProLnk contractor — unique per person, expires after job completion. Access log shows exact entry/exit times. DFW homes with pools or detached garages: add a secondary keypad at gate ($120-$200). ProLnk contractors receive PIN via secure SMS — no physical key handoff required.',
    gate: '🚗 Gate Access System: For DFW properties with driveway gates, LiftMaster 841LM Smart Control Panel ($130) adds WiFi to existing gate openers. Generate temporary virtual keys via myQ app. For new gate installs, Linear ACP00877 with keypad + remote ($250) handles DFW HOA pass-through requirements. Add a video intercom (Doorbird D101S, $350) to verify visitors before remote gate release.',
    multientry: '🏠 Multi-Door Access: For DFW homes with garage man-door, side entrance, and back gate — August Wi-Fi Smart Lock (4th Gen) on each door ($230/door) + August Access app creates a unified access dashboard. Assign different PINs per contractor per door. Elimu or Yale network hub ($80) links all locks. Access audit log exports to CSV for property management records.',
    intercom: '📹 Video Intercom System: Doorbird D2101V ($400) replaces your DFW doorbell with HD video intercom — visitors ring, you answer from anywhere via app, unlock remotely. For gated DFW estates, 2N Helios IP Verso ($1,200) handles outdoor intercom at gate + secondary indoor panel. Integrates with Alexa and Google Home for hands-free answer.',
    fullsystem: '🔐 Full Access Control System (DFW Estate): Commercial-grade Brivo or Salto KS access control. Each door gets a card/fob/mobile credential reader ($300/door). Web dashboard manages all users, doors, schedules. DFW property managers use this for rental properties — generate/revoke credentials remotely, run occupancy reports. Monthly SaaS fee: $50-$120 depending on door count.',
  };
