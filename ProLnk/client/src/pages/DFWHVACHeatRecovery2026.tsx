      ...
      <div style={{ maxWidth: 720, margin: '0 auto' }}>
        ...
        <div style={{ background: '#0F1F3D', borderRadius: 16, padding: 28, border: '1px solid #1e3a5f', marginBottom: 28 }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 10, marginBottom: 20 }}>
            {situations.map(s => (
              <button key={s.value} onClick={() => setSituation(s.value)} style={{ padding: '12px 16px', borderRadius: 8, border: 'none', background: situation === s.value ? '#F5E642' : '#0A1628', color: situation === s.value ? '#0A1628' : '#fff', cursor: 'pointer', textAlign: 'left', fontWeight: 600 }}>{s.label}</button>
            ))}
          </div>
