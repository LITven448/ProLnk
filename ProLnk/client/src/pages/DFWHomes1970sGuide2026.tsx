...
const issues = [
  { id: 'polybutylene', label: '🚰 Polybutylene Pipes', desc: 'Quest/PB pipes — recalled, failure prone' },
...
export default function DFWHomes1970sGuide2026() {
    ...
    <div style={{ background: '#0A1628', minHeight: '100vh', padding: '32px 16px', fontFamily: 'system-ui, sans-serif', color: '#E8EDF5' }}>
      <div style={{ maxWidth: 720, margin: '0 auto' }}>
        <div style={{ background: '#F5E642', color: '#0A1628', borderRadius: 12, padding: '24px', marginBottom: 32 }}>
          ...
          <h1 style={{ margin: '8px 0 4px', fontSize: 26, fontWeight: 700 }}>DFW 1970s Home Owner Guide 2026</h1>
          <p style={{ margin: 0, fontSize: 14, opacity: 0.8 }}>Built 1970–1979 · DFW Metro · Updated May 2026</p>
        </div>

        <div style={{ background: '#111E35', borderRadius: 12, padding: 24, marginBottom: 24 }}>
          <h2 style={{ color: '#F5E642', marginTop: 0, fontSize: 18 }}>⚠️ Known Issues — Does Your Home Have These?</h2>
          <p style={{ color: '#9EB0CC', fontSize: 14, marginBottom: 16 }}>1970s DFW homes have specific known problem systems. Check yours.</p>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
            {issues.map(i => (
              <div key={i.id} onClick={() => toggle(i.id)}
                ...
                <div style={{ fontSize: 22 }}>{selected.includes(i.id) ? '✅' : '⬜'}</div>
              </div>
            ))}
          </div>
          {selected.length > 0 && (
            <button onClick={() => setShowGuide(true)}
              ...
              🔧 Get My 1970s Priority Repair Guide
            </button>
          )}
        </div>
        ...
        {showGuide && selected.length > 0 && (
          <div style={{ background: '#111E35', borderRadius: 12, padding: 24, marginBottom: 24 }}>
            <h2 style={{ color: '#F5E642', marginTop: 0 }}>🔧 Your 1970s Priority Repair Guide</h2>
            {selected.map(id => {
              const r = repairs[id];
              return (
                <div key={id} style={{ background: '#0D1E38', borderRadius: 10, padding: 16, marginBottom: 12, borderLeft: '4px solid #F5E642' }}>
                  ...
                  <div style={{ color: '#F5E642', fontSize: 13, marginBottom: 4 }}>Est. Cost: {r.cost}</div>
                  <div style={{ color: '#9EB0CC', fontSize: 13 }}>{r.note}</div>
                </div>
              );
            })}
