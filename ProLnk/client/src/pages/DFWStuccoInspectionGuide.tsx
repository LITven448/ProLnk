...
const ASSESSMENTS: Record<string, Record<string, { assessment: string; inspectionType: string; cost: string }>> = {
  soft: {
    pre1980: { assessment: 'High risk. Pre-1980 DFW stucco is often 3-coat Portland cement over wood lath. Soft spots = moisture has been there long enough to rot lath. Likely remediation required.', inspectionType: 'Hire certified stucco inspector with moisture meter. Probe at multiple points. Do not patch over soft areas.', cost: 'Inspection: -700. Remediation estimate will likely be ,000-25,000+ depending on extent.' },
    post1990: { assessment: 'Post-1990 likely EIFS (synthetic stucco). Soft EIFS is a serious moisture intrusion signal. EIFS moisture damage in DFW is very common and very expensive.', inspectionType: 'EIFS-specific inspector required. Probe test at all transitions, windows, and penetrations. Infrared scan valuable.', cost: 'EIFS inspection: -900. Remediation: ,000-40,000+ for full EIFS replacement.' },
    new: { assessment: 'New construction with soft stucco is a defect. Builder warranty claim should be filed immediately. Do not allow builder to simply repaint over defect.', inspectionType: 'Independent inspector (not builder-affiliated). Document with photos and dates.', cost: 'Independent inspection: -600. Builder warranty should cover remediation.' },
  },
  cracks: {
    pre1980: { assessment: 'Traditional 3-coat stucco develops a crack network over decades — not always a failure. Key question: are cracks sealed or open to DFW rain?', inspectionType: 'DIY probe test on major cracks. If probe goes behind stucco more than 1 inch, hire inspector.', cost: 'DIY elastomeric seal: -400. Pro inspection if probe test fails: -600.' },
    post1990: { assessment: 'If EIFS, any crack is a moisture entry risk. DFW summer rain intensity means even small EIFS cracks can allow significant moisture accumulation.', inspectionType: 'Do not DIY patch EIFS cracks — seal must be system-compatible. Hire EIFS contractor.', cost: 'EIFS crack repair: -1,200 per area. Prevent by maintaining caulk joints.' },
    new: { assessment: 'Cracks in stucco under 5 years old suggest improper installation. Check scratch coat and brown coat application method.', inspectionType: 'Builder callback first. If dismissed, hire independent inspector for defect documentation.', cost: 'Independent inspection: -600. Builder warranty dispute if defect confirmed.' },
  },
  discolor: {
    pre1980: { assessment: 'Brown or dark staining on old stucco is often organic growth (algae, mold) from DFW humidity, or rust bleed from embedded metal lath. Both require investigation.', inspectionType: 'Visual inspection + moisture meter reading behind stain. Probe if meter reads above 16%.', cost: 'Moisture meter test: DIY with -80 meter. If wet behind stucco, inspection: -600.' },
    post1990: { assessment: 'Dark staining on EIFS is a major warning sign — EIFS retains moisture at transitions. Efflorescence (white staining) means water is moving through the system.', inspectionType: 'Hire EIFS inspector. Staining pattern maps water path — do not just clean and repaint.', cost: 'EIFS inspection: -900. Staining without moisture: cleaning -600.' },
    new: { assessment: 'Staining on new stucco is typically construction residue, form release agent, or mineral migration. Usually cosmetic in year 1-2.', inspectionType: 'DIY assessment first — try stucco cleaner. If stain returns, hire inspector.', cost: 'Stucco cleaner + DIY: -150. Pro cleaning: -700.' },
  },
};
