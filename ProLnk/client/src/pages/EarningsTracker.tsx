...
export default function EarningsTracker() {
  ...
  const ytdPaid = useMemo(() => {
    ...
    return commissions
      .filter((c: any) => c.paid && new Date(c.createdAt) >= yearStart)
      .reduce((s: number, c: any) => s + Number(c.amount ?? 0), 0);
  }, [commissions]);

  const handle1099Export = () => {
    if (!commissions || commissions.length === 0) { toast.error("No paid earnings to export"); return; }
    const year = new Date().getFullYear();
    const yearStart = new Date(year, 0, 1);
    const ytdCommissions = commissions.filter((c: any) => c.paid && new Date(c.createdAt) >= yearStart);
    if (ytdCommissions.length === 0) { toast.error(`No paid earnings in ${year}`); return; }
    const rows = [
      ["Payer Name", "Payer EIN", "Recipient", "Recipient Email", "Box 1 - Nonemployee Compensation", "Tax Year"],
      ...
      ["", "", "", "TOTAL YTD", ytdPaid.toFixed(2), year],
    ];
    ...
    toast.success(`1099-NEC data exported for ${year}`);
  };
