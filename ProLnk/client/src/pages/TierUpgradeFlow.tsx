...
export default function TierUpgradeFlow() {
  ...
  if (celebrating) {
    const tierName = new URLSearchParams(window.location.search).get("tier") ?? "Pro";
    return (
      <PartnerLayout>
        <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-[#0A1628] to-[#1B4FD8] p-6">
          <div className="text-center max-w-lg">
            <div className="text-7xl mb-6 animate-bounce">🎉</div>
            <h1 className="text-4xl font-black text-white mb-3">You're Upgraded!</h1>
            <p className="text-xl text-white/70 mb-2">Welcome to <span className="text-[#F5E642] font-bold">{tierName}</span></p>
            <p className="text-white/50 text-sm mb-10">Your new commission rates and features are active immediately. Go make money.</p>
            <div className="grid grid-cols-3 gap-4 mb-10">
              {[
                { label: "Higher commission split", icon: "💰" },
                { label: "Priority lead routing", icon: "⚡" },
                { label: "Unlimited referrals", icon: "🌐" },
              ].map((item) => (
            ...
            <div className="flex gap-3 justify-center flex-wrap">
              <Button
                ...
                className="bg-[#F5E642] text-[#0A1628] font-bold hover:bg-[#F5E642]/90 px-8"
              >
                Go to Dashboard
              </Button>
              <Button
