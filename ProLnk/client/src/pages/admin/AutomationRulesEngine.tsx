  ...
  return (
    <AdminLayout title="Automation Rules Engine" subtitle="Event-driven rules that run automatically across the platform">
      ...
      <div className="grid grid-cols-4 gap-4 mb-6″>
        {[
          { label: "Total Rules", value: stats?.total || rules.length, color: "text-white" },
          { label: "Active", value: stats?.active || rules.filter(r => r.isActive).length, color: "text-teal-400″ },
          { label: "Total Executions", value: (stats?.totalExecutions || rules.reduce((s, r) => s + r.executionCount, 0)).toLocaleString(), color: "text-amber-400″ },
          { label: "Avg Daily Runs", value: Math.round((stats?.totalExecutions || 244) / 30), color: "text-purple-400″ },
        ].map(s => (
        ...
        <div className="flex gap-1 bg-[#0D1F3C] rounded-xl p-1 border border-white/10″>
          {(["rules", "history", "templates"] as Tab[]).map(t => (
            <button key={t} onClick={() => setActiveTab(t)}
              className={`text-xs font-medium py-2 px-4 rounded-lg capitalize transition-all ${activeTab === t ? "bg-teal-500/20 text-teal-400" : "text-slate-400 hover:text-white"}`}>
              {t === "history" ? "Execution History" : t === "templates" ? "Templates" : "Active Rules"}
            </button>
          ))}
        </div>
        <Button onClick={() => setShowCreate(true)} className="gap-2 text-sm bg-teal-600 hover:bg-teal-700 text-white">
        ...
        <div className="space-y-3″>
          ...
          <div className="flex items-center gap-2″>
            ...
            <select
              ...
              className="border border-white/10 rounded-lg px-3 py-1.5 text-xs bg-[#0D1F3C] text-white focus:outline-none focus:ring-2 focus:ring-teal-500″
            >
              <option value="all">All Triggers</option>
              {Object.entries(TRIGGER_LABELS).map(([k, v]) => <option key={k} value={k}>{v}</option>)}
            </select>
            <span className="text-xs text-slate-500″>{displayRules.length} rules</span>
          </div>
                ...
                <div key={rule.id} className={`rounded-xl border ${rule.isActive ? "border-white/10" : "border-white/5 opacity-60"} bg-[#0D1F3C] p-4`}>
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-4 flex-1 min-w-0″>
                      <div className={`p-2 rounded-lg flex-shrink-0 ${rule.isActive ? "bg-teal-500/20" : "bg-slate-700/50"}`}>
                        <TriggerIcon className={`w-5 h-5 ${rule.isActive ? "text-teal-400" : "text-slate-500"}`} />
                      </div>
                      <div className="flex-1 min-w-0″>
                        <div className="flex items-center gap-2 mb-0.5″>
                          <h3 className="font-medium text-white text-sm truncate">{rule.name}</h3>
                          <span className={`text-xs px-2 py-0.5 rounded-full flex-shrink-0 ${rule.isActive ? "bg-teal-500/20 text-teal-400" : "bg-slate-500/20 text-slate-400"}`}>
                            {rule.isActive ? "Active" : "Paused"}
                          </span>
                        </div>
                        {rule.description && <p className="text-xs text-slate-400 truncate">{rule.description}</p>}
                        <div className="flex items-center gap-2 mt-2 text-xs text-slate-500 flex-wrap">
                          ...
                          {(rule.conditionField || rule.conditionValue) && (
                            <>
                              ...
                              <span className="font-mono bg-white/5 px-1.5 py-0.5 rounded text-slate-400″>
                          ...
                          {rule.lastRun && <><span className="text-slate-600″>·</span><span>Last: {rule.lastRun}</span></>}
                        </div>
                      </div>
                    </div>
                    <div className="flex items-center gap-2 ml-4 flex-shrink-0″>
                      <Button
                        size="sm"
                        variant="ghost"
                        className="h-7 text-xs text-slate-400 hover:text-white hover:bg-white/5 gap-1″
      ...
      {activeTab === "history" && (
        <div className="rounded-xl border border-white/10 bg-[#0D1F3C]">
          <div className="p-4 border-b border-white/10 flex items-center gap-2″>
            ...
            <h3 className="font-bold text-white text-sm">Execution History</h3>
            <span className="text-xs text-slate-500 ml-auto">Last 50 runs</span>
          </div>
                ...
                {EXECUTION_HISTORY.map(e => {
                  ...
                  return (
                    <tr key={e.id} className="border-b border-white/5 hover:bg-white/2″>
                      <td className="px-4 py-3 font-medium text-white text-xs">{e.ruleName}</td>
                      ...
                      <td className="px-4 py-3 text-xs text-slate-400″>{e.target}</td>
                      <td className="px-4 py-3″>
                        <span className={`text-xs px-2 py-0.5 rounded-full ${e.status === "success" ? "bg-emerald-500/20 text-emerald-400" : "bg-red-500/20 text-red-400"}`}>
                          {e.status}
                        </span>
                      </td>
                      <td className="px-4 py-3 text-xs text-slate-400″>{e.ranAt}</td>
                    </tr>
                  );
                })}
              </tbody>
      ...
      {activeTab === "templates" && (
        <div className="space-y-3″>
          <p className="text-xs text-slate-400″>Pre-built automation templates. Click any to create a rule from it.</p>
                ...
                >
                  <div className="flex items-start gap-3″>
                    <div className="p-2 rounded-lg bg-amber-500/20 flex-shrink-0″>
                      <TriggerIcon className="w-5 h-5 text-amber-400″ />
                    </div>
                    <div className="flex-1 min-w-0″>
                      <h3 className="font-medium text-white text-sm group-hover:text-teal-400 transition-colors">{t.name}</h3>
                      ...
                      <div className="mt-1.5 font-mono text-xs bg-white/5 px-2 py-1 rounded text-slate-400 inline-block">{t.condition}</div>
                    </div>
                  </div>
                </div>
              );
            ...
            {templates.map((template: any, i: number) => {
              ...
              return (
                <div key={`db-${i}`} className="rounded-xl border border-white/10 bg-[#0D1F3C] p-4 hover:border-teal-500/40 cursor-pointer transition-colors group"
                  ...
                  <div className="flex items-start gap-3″>
                    <div className="p-2 rounded-lg bg-teal-500/20″><TriggerIcon className="w-5 h-5 text-teal-400" /></div>
                    <div>
          ...
          <div className="rounded-2xl border border-white/10 bg-[#0D1F3C] w-full max-w-lg max-h-[90vh] overflow-y-auto">
            <div className="flex items-center justify-between p-5 border-b border-white/10″>
              <div className="flex items-center gap-2″>
                ...
                <h2 className="font-bold text-white">Create Automation Rule</h2>
              </div>
            ...
            <div className="p-5 space-y-4″>
              <div>
                ...
                <Input value={newRule.name} onChange={e => setNewRule({ ...newRule, name: e.target.value })}
                  className="bg-[#0A1628] border-white/10 text-white focus:border-teal-500″ placeholder="e.g., Auto-Route High-Score Leads" />
              </div>
              <div>
                <label className="block text-xs font-medium text-slate-400 mb-1″>Description</label>
                <Textarea value={newRule.description} onChange={e => setNewRule({ ...newRule, description: e.target.value })}
                  className="bg-[#0A1628] border-white/10 text-white focus:border-teal-500″ placeholder="What does this rule do?" rows={2} />
              </div>
              ...
              <div>
                ...
                <div className="grid grid-cols-3 gap-2″>
                  <select value={newRule.conditionField} onChange={e => setNewRule({ ...newRule, conditionField: e.target.value })}
                    className="border border-white/10 rounded-lg px-2 py-2 text-xs bg-[#0A1628] text-white focus:outline-none focus:ring-2 focus:ring-teal-500″>
                  ...
                  <Input value={newRule.conditionValue} onChange={e => setNewRule({ ...newRule, conditionValue: e.target.value })}
                    className="bg-[#0A1628] border-white/10 text-white focus:border-teal-500 text-xs" placeholder="value" />
                </div>
                <div className="mt-2 text-xs font-mono bg-white/5 rounded px-2 py-1 text-slate-400″>
              ...
              {/* Action Config */}
              <div>
                <label className="block text-xs font-medium text-slate-400 mb-1″>Action Config (optional JSON)</label>
                <Textarea value={newRule.actionConfigJson} onChange={e => setNewRule({ ...newRule, actionConfigJson: e.target.value })}
                  className="bg-[#0A1628] border-white/10 text-white font-mono text-xs focus:border-teal-500″
                  placeholder='{"template": "welcome_email", "delay_minutes": 0}' rows={3} />
              </div>

              <div className="flex items-center justify-between p-3 rounded-lg bg-white/5″>
                <div>
                  <div className="text-sm font-medium text-white">Activate immediately</div>
                  <div className="text-xs text-slate-400″>Rule will run as soon as events fire</div>
                </div>
            ...
            <div className="flex items-center justify-end gap-3 p-5 border-t border-white/10″>
              ...
              <Button onClick={createRule} disabled={!newRule.name || createMutation.isPending} className="bg-teal-600 hover:bg-teal-700 text-white">
