...
function riskBadge(score: number) {
  if (score > 70) return <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded text-xs font-semibold bg-red-500/20 text-red-600">{score}</span>;
...
function riskLabel(score: number) {
  if (score > 70) return <Badge className="bg-red-500/20 text-red-600 border-0">High</Badge>;
      ...
      <div className="p-6 space-y-6 bg-[#F8FAFC] min-h-screen">
        <div className="flex items-start justify-between">
          <div>
            ...
            <p className="text-gray-500 mt-1">Pros at risk of going inactive based on activity signals</p>
          </div>
          <Button onClick={exportCSV} variant="outline" size="sm" className="border-gray-200 text-gray-700 hover:text-gray-900">
          ...
          <Card className="bg-white border-gray-200">
            <CardContent className="pt-5">
              <div className="flex items-center gap-2 mb-1">
                ...
                <span className="text-xs text-gray-500 uppercase tracking-wider">Total At-Risk</span>
              </div>
              <div className="text-3xl font-bold text-gray-900">{totalAtRisk.length}</div>
              <p className="text-xs text-gray-500 mt-1">Score 30 or above</p>
            </CardContent>
          </Card>
          <Card className="bg-white border-red-900/40 border">
            <CardContent className="pt-5">
              <div className="flex items-center gap-2 mb-1">
                <AlertTriangle className="h-4 w-4 text-red-600" />
                <span className="text-xs text-gray-500 uppercase tracking-wider">High Risk</span>
              </div>
              <div className="text-3xl font-bold text-red-600">{highRisk.length}</div>
              <p className="text-xs text-gray-500 mt-1">Score above 70</p>
            </CardContent>
          </Card>
          <Card className="bg-white border-gray-200">
            <CardContent className="pt-5">
              <div className="flex items-center gap-2 mb-1">
                ...
                <span className="text-xs text-gray-500 uppercase tracking-wider">Revenue at Risk</span>
              </div>
              <div className="text-3xl font-bold text-yellow-400">${revenueAtRisk.toLocaleString()}</div>
              <p className="text-xs text-gray-500 mt-1">Monthly from high-risk pros</p>
            </CardContent>
          </Card>
        </div>
        ...
        <Card className="bg-white border-gray-200">
          <CardHeader className="pb-3">
            <div className="flex flex-col sm:flex-row gap-3 items-start sm:items-center justify-between">
              <CardTitle className="text-gray-900 text-lg">At-Risk Pros</CardTitle>
              <div className="flex flex-wrap gap-2">
                <div className="relative">
                  ...
                  <Input
                    ...
                    className="pl-8 h-8 w-44 bg-gray-50 border-gray-200 text-sm text-gray-900 placeholder:text-gray-500"
                  />
                </div>
                <Select value={tradeFilter} onValueChange={setTradeFilter}>
                  <SelectTrigger className="h-8 w-36 bg-gray-50 border-gray-200 text-sm text-gray-900">
                    <SelectValue />
                  </SelectTrigger>
                  <SelectContent className="bg-gray-50 border-gray-200">
                    {TRADES.map((t) => (
                      <SelectItem key={t} value={t} className="text-gray-700">{t}</SelectItem>
                    ))}
                  </SelectContent>
                </Select>
                <Select value={riskFilter} onValueChange={setRiskFilter}>
                  <SelectTrigger className="h-8 w-44 bg-gray-50 border-gray-200 text-sm text-gray-900">
                    <SelectValue />
                  </SelectTrigger>
                  <SelectContent className="bg-gray-50 border-gray-200">
                    {RISK_LEVELS.map((r) => (
                      <SelectItem key={r} value={r} className="text-gray-700">{r}</SelectItem>
                    ))}
                  </SelectContent>
                </Select>
              </div>
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-sm">
                ...
                <tbody>
                  {active.length === 0 && (
                    <tr>
                      <td colSpan={7} className="py-10 text-center text-gray-500">No pros match current filters</td>
                    </tr>
                  )}
                  {active.map((pro) => (
                    <tr key={pro.id} className="border-b border-gray-200/50 hover:bg-gray-100/20 transition-colors">
                      <td className="py-3 pr-4 font-medium text-gray-900">{pro.name}</td>
                      <td className="py-3 pr-4 text-gray-700">{pro.trade}</td>
                      <td className="py-3 pr-4 text-gray-500">{pro.lastActive}</td>
                      <td className="py-3 pr-4 text-right text-gray-700">{pro.jobsLast30}</td>
                      ...
                      <td className="py-3 text-right">
                        <div className="flex items-center justify-end gap-1">
                          <Button
                            size="sm"
                            variant="ghost"
                            className="h-7 px-2 text-xs text-teal-700 hover:text-teal-700 hover:bg-teal-400/10"
                          ...
                          <Button
                            size="sm"
                            variant="ghost"
                            className="h-7 px-2 text-xs text-blue-400 hover:text-teal-700 hover:bg-blue-400/10"
                  ...
                  {pros.filter((p) => p.status === "resolved").length > 0 && (
                    <tr>
                      <td colSpan={7} className="pt-4 pb-1 text-xs text-gray-500 uppercase tracking-wider">
                  ...
                  {pros.filter((p) => p.status === "resolved").map((pro) => (
                    <tr key={`resolved-${pro.id}`} className="opacity-40">
                      <td className="py-2 pr-4 text-gray-500 line-through">{pro.name}</td>
                      <td className="py-2 pr-4 text-gray-500">{pro.trade}</td>
                      <td className="py-2 pr-4 text-gray-500">{pro.lastActive}</td>
                      <td className="py-2 pr-4 text-right text-gray-500">{pro.jobsLast30}</td>
                      <td className="py-2 pr-4 text-center"><Badge className="bg-gray-100 text-gray-500 border-0">Resolved</Badge></td>
                      <td className="py-2 pr-4 text-center text-gray-500">{pro.riskScore}</td>
                      <td className="py-2" />
                    </tr>
                  ))}
                </tbody>
