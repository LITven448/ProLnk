        ...
        <Card className="bg-white border-gray-200 shadow-sm">
          <CardHeader className="pb-3">
            <div className="flex items-center justify-between flex-wrap gap-3">
              ...
              <div className="flex items-center gap-2">
                {(payoutRequests as any[]).filter((r: any) => r.status === 'pending').length > 1 && (
                  <button
                    onClick={async () => {
                      const pending = (payoutRequests as any[]).filter((r: any) => r.status === 'pending');
                      if (!confirm(`Approve all ${pending.length} pending payout requests? This will trigger Stripe transfers for each.`)) return;
                      let ok = 0; let fail = 0;
                      for (const req of pending) {
                        try {
                          await reviewMutation.mutateAsync({ requestId: req.id, action: 'approved' });
                          ok++;
                        } catch { fail++; }
                      }
