...
function SectionHeader({ icon, title, subtitle }: { icon: React.ReactNode; title: string; subtitle?: string }) {
  return (
    <div className="flex items-start gap-3 mb-5">
      <div className="w-9 h-9 rounded-xl bg-gray-100 flex items-center justify-center flex-shrink-0 text-teal-700">
        {icon}
      </div>
      <div>
...
function NotificationRow({ row, onChange }: {
  ...
  return (
    <div className="flex items-center gap-3 py-2.5 border-b border-gray-200 last:border-0">
      <span className="flex-1 text-xs text-gray-700">{row.label}</span>
      {(["email", "sms", "push"] as const).map(ch => (
        ...
        >
          ...
          <span className="text-[9px] text-gray-400 uppercase">{ch}</span>
        </button>
      ))}
    </div>
  );
...
export default function PlatformSettings() {
  ...
  const Section = ({ id, title, children }: { id: string; title: string; children: React.ReactNode }) => {
    ...
    return (
      <div className="bg-[#0F1E35] rounded-2xl border border-gray-200/50 overflow-hidden mb-4">
        <button
        ...
        <div className="max-w-3xl mx-auto">
          ...
          <div className="mb-6">
            <h1 className="text-2xl md:text-3xl font-bold text-gray-900 flex items-center gap-2">
              <Settings className="w-7 h-7 text-teal-700" /> Platform Settings
            </h1>
            <p className="text-gray-500 mt-1 text-sm">Feature flags, commission rates, routing, and notification configuration</p>
          </div>

          <div className="bg-[#0F1E35] rounded-2xl border border-gray-200/50 p-5 mb-4">
            <SectionHeader
                ...
                return (
                  <button
                    ...
                    className={[
                      ...
                      active ? `${cfg.bg} ${cfg.border}` : "bg-transparent border-gray-200/50 hover:border-gray-300",
                    ].join(" ")}
                  >
                    <div className={`text-sm font-bold ${active ? cfg.color : "text-gray-500"}`}>{cfg.label}</div>
                    <div className="text-[10px] text-gray-500 mt-0.5 leading-tight">{cfg.description}</div>
                  </button>
                );
              })}
            </div>
            {pendingMode && (
              <div className="bg-amber-900/20 border border-amber-700/40 rounded-xl p-3 flex flex-col sm:flex-row sm:items-center gap-3">
                ...
                <div className="flex gap-2">
                  ...
                  <Button size="sm" variant="outline" onClick={() => setPendingMode(null)} className="border-gray-300 text-gray-500 bg-transparent text-xs">
                    Cancel
                  </Button>
                </div>
              </div>
          ...
          <Section id="flags" title="Feature Flags">
            <div className="flex flex-col gap-0">
              {flags.map(f => (
                <div key={f.key} className="flex items-center justify-between py-3 border-b border-gray-200 last:border-0">
                  ...
                  <button onClick={() => toggleFlag(f.key)} className="flex items-center gap-2">
                    ...
                    <span className={`text-xs font-semibold w-6 ${f.enabled ? "text-teal-700" : "text-gray-400"}`}>
                      {f.enabled ? "ON" : "OFF"}
                    </span>
                  </button>
                </div>
          ...
          <Section id="commission" title="Commission Settings">
            <div className="flex items-center gap-2 mb-4 p-3 bg-white/50 rounded-xl border border-gray-200/40">
              <Lock className="w-4 h-4 text-gray-500 flex-shrink-0" />
              <p className="text-xs text-gray-500">These rates are locked by engineering. Contact engineering to change rates.</p>
            </div>
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
              {[
                { label: "Partner Keep Rate", value: "60%", color: "text-teal-700" },
                ...
                { label: "Monthly Subscription", value: "$149/mo", color: "text-amber-700" },
                { label: "Sub Override L1", value: "12%", color: "text-gray-600" },
                { label: "Sub Override L2", value: "6%", color: "text-gray-600" },
                { label: "Origination Rights", value: "1.5%", color: "text-green-700" },
                { label: "Min Payout", value: "$25", color: "text-gray-500" },
                { label: "Payout Cycle", value: "Monthly", color: "text-gray-500" },
                { label: "Waitlist Cap", value: "500 apps", color: "text-orange-400" },
              ].map(item => (
                <div key={item.label} className="bg-[#F8FAFC] rounded-xl border border-gray-200 p-3">
          ...
          <Section id="leads" title="Lead Settings">
            <div className="grid grid-cols-2 gap-3">
              {[
                ...
                <div key={item.label} className="bg-[#F8FAFC] rounded-xl border border-gray-200 p-3 flex items-center justify-between gap-2">
                  <div>
                  ...
                  {item.edit && (
                    <button className="text-[10px] text-teal-700 border border-teal-700/40 rounded px-1.5 py-0.5 hover:bg-teal-900/30">
                      Edit
                    </button>
                  )}
                </div>
          ...
          <Section id="notifications" title="Notification Settings">
            <div className="mb-4">
              <div className="flex items-center gap-2 mb-3">
                ...
                <h3 className="text-xs font-semibold text-gray-700 uppercase tracking-wide">Partner Notifications</h3>
                <div className="ml-auto flex gap-4 text-[10px] text-gray-400 uppercase">
            ...
            <div>
              <div className="flex items-center gap-2 mb-3 mt-5">
                ...
                <h3 className="text-xs font-semibold text-gray-700 uppercase tracking-wide">Homeowner Notifications</h3>
                <div className="ml-auto flex gap-4 text-[10px] text-gray-400 uppercase">
                  <span className="flex items-center gap-1"><Mail className="w-3 h-3" /> Email</span>
                  <span className="flex items-center gap-1"><MessageSquare className="w-3 h-3" /> SMS</span>
                  <span className="flex items-center gap-1"><Smartphone className="w-3 h-3" /> Push</span>
          ...
          <div className="bg-[#0F1E35] rounded-2xl border border-gray-200/50 p-5">
            <SectionHeader
            ...
            <div className="flex flex-wrap gap-3">
              <Button variant="outline" className="border-red-700/50 text-red-600 hover:bg-red-900/20 bg-transparent text-xs flex items-center gap-1.5">
