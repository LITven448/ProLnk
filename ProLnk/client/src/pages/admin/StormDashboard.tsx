...
import {
  ...
  TrendingUp, Activity, Radio, Settings, ArrowUpRight, Eye, ToggleLeft, ToggleRight,
} from "lucide-react";
...
const HEAT_MAP_ZIPS = [
  { zip: "75034", area: "Frisco",      level: "red"   },
  { zip: "75035", area: "Frisco E",    level: "red"   },
  { zip: "75013", area: "Allen",       level: "amber" },
  { zip: "75023", area: "Plano N",     level: "amber" },
  { zip: "75024", area: "Plano NW",    level: "amber" },
  { zip: "75025", area: "Plano NE",    level: "amber" },
  { zip: "75201", area: "Dallas Core", level: "yellow"},
  { zip: "75202", area: "Dallas DT",   level: "yellow"},
  { zip: "75204", area: "Uptown",      level: "yellow"},
  { zip: "76101", area: "Fort Worth",  level: "green" },
  { zip: "76010", area: "Arlington",   level: "green" },
  { zip: "76201", area: "Denton",      level: "green" },
];

const HEAT_MAP_STYLE: Record<string, { bg: string; text: string; border: string; label: string }> = {
  red:    { bg: "bg-red-600/20",    text: "text-red-300",    border: "border-red-500/60",    label: "bg-red-600"    },
  amber:  { bg: "bg-amber-500/15",  text: "text-amber-300",  border: "border-amber-500/40",  label: "bg-amber-500"  },
  yellow: { bg: "bg-yellow-500/10", text: "text-yellow-300", border: "border-yellow-400/30", label: "bg-yellow-400" },
  green:  { bg: "bg-teal-600/10",   text: "text-teal-300",   border: "border-teal-600/20",   label: "bg-teal-500"   },
};

const TICKER_ITEMS = [
  "Hail warning active: Frisco 75034 — 14 leads auto-generated",
  "Tornado watch lifted: Denton Co. 76201 — 3 leads pending dispatch",
  "Flash flood advisory: Dallas Core 75201 — 8 leads created",
  "Wind advisory: Fort Worth 76101 — 5 roofing pros notified",
  "Severe thunderstorm: Allen 75013 — 11 leads in queue",
];

const STORM_HISTORY_DATA = [
  { month: "Dec", events: 2, leads: 89  },
  { month: "Jan", events: 4, leads: 187 },
  { month: "Feb", events: 3, leads: 142 },
  { month: "Mar", events: 6, leads: 521 },
  { month: "Apr", events: 8, leads: 699 },
  { month: "May", events: 5, leads: 412 },
];

const LEAD_GEN_LOG = [
  { date: "May 13, 2:18am", event: 'Hail 1.25" in Frisco',      created: 47,  dispatched: 31,  pending: 16 },
  { date: "May 11, 7:44pm", event: "Severe tstorm — Allen",      created: 28,  dispatched: 28,  pending: 0  },
  { date: "May 9, 11:03am", event: "Wind 58mph — Fort Worth",    created: 34,  dispatched: 22,  pending: 12 },
  { date: "May 8, 3:55am",  event: "Supercell — DFW Metroplex", created: 412, dispatched: 387, pending: 25 },
  { date: "May 6, 1:20pm",  event: "Flash Flood — Dallas Core",  created: 19,  dispatched: 19,  pending: 0  },
];
  ...
  useEffect(() => {
    ...
    DFW_ZONES.forEach((zone, i) => {
      ...
      fetch(url).then(r => r.json()).then(json => {
        ...
        setZones(prev => {
          const next = [...prev];
          next[i] = { zone, windSpeed: Math.round(wind), temp: Math.round(vals.temperature ?? 0), precipProb: vals.precipitationProbability ?? 0, precipIntensity: precip, weatherCode: code, threatLevel: getThreatLevel(wind, precip, code), loading: false };
          return next;
        });
      }).catch(() => {});
    });
    ...
    <div className={`rounded-xl border p-4 transition-all ${style.border} ${style.bg}`}>
      ...
      <div className="grid grid-cols-3 gap-2">
        <div className="text-center">
          <div className="flex items-center justify-center mb-0.5"><Thermometer className="w-3 h-3 text-amber-400" /></div>
          <p className="text-sm font-bold text-white">{zone.temp}°F</p>
          <p className="text-[9px] text-gray-500">Temp</p>
        </div>
        <div className="text-center">
          <div className="flex items-center justify-center mb-0.5"><Wind className={`w-3 h-3 ${zone.windSpeed > 40 ? "text-red-400" : "text-blue-400"}`} /></div>
        ...
        <div className="text-center">
          <div className="flex items-center justify-center mb-0.5"><Droplets className="w-3 h-3 text-blue-400" /></div>
...
function StormTicker() {
  const [pos, setPos] = useState(0);
  const text = TICKER_ITEMS.join("     ·     ");
  const width = text.length * 7;
  useEffect(() => {
    const id = setInterval(() => setPos(p => (p - 1) % -width), 30);
    return () => clearInterval(id);
  }, [width]);
  return (
    <div className="overflow-hidden bg-red-950/40 border border-red-500/30 rounded-lg px-4 py-2 flex items-center gap-3">
      <span className="text-[10px] font-bold text-red-400 uppercase tracking-widest flex-shrink-0 border border-red-500/40 px-1.5 py-0.5 rounded">LIVE</span>
      <div className="overflow-hidden flex-1">
        <div className="whitespace-nowrap text-xs text-red-200" style={{ transform: `translateX(${pos}px)`, transition: "none" }}>
          {text} &nbsp;&nbsp;&nbsp;&nbsp; {text}
        </div>
      </div>
    </div>
    ...
    <Card>
      <CardHeader className="pb-3">
        <CardTitle className="text-base flex items-center gap-2">
          ...
          DFW ZIP Zone Severity Heat Map
          <span className="ml-auto text-[10px] text-muted-foreground font-normal">Updated 2 min ago</span>
        </CardTitle>
      </CardHeader>
      <CardContent>
        <div className="grid grid-cols-4 gap-2 mb-3">
          {HEAT_MAP_ZIPS.map(z => {
            const s = HEAT_MAP_STYLE[z.level];
            return (
              <div key={z.zip} className={`rounded-lg border p-2.5 text-center ${s.bg} ${s.border}`}>
                <p className={`text-xs font-bold ${s.text}`}>{z.zip}</p>
                <p className="text-[9px] text-muted-foreground mt-0.5 leading-tight">{z.area}</p>
                <span className={`mt-1 inline-block w-2 h-2 rounded-full ${s.label}`} />
              </div>
            );
          })}
        </div>
        <div className="flex items-center gap-4 text-[10px] text-muted-foreground pt-2 border-t border-border">
          {[["bg-red-600","Active warning"],["bg-amber-500","Watch"],["bg-yellow-400","Advisory"],["bg-teal-500","Clear"]].map(([cls, lbl]) => (
...
function AutoDispatchToggle({ enabled, onToggle }: { enabled: boolean; onToggle: () => void }) {
  return (
    <div className={`flex items-center justify-between px-5 py-3.5 rounded-xl border transition-colors ${enabled ? "bg-green-500/8 border-green-500/30" : "bg-muted/30 border-border"}`}>
      <div className="flex items-center gap-3">
        <Zap className={`w-4 h-4 ${enabled ? "text-green-400" : "text-muted-foreground"}`} />
        <div>
          <p className="text-sm font-semibold text-foreground">Auto-dispatch storm leads to active roofing pros</p>
          <p className="text-xs text-muted-foreground">When a hail or wind event is detected, leads are immediately routed</p>
        </div>
      </div>
      <button onClick={onToggle} className="flex items-center gap-2 flex-shrink-0 ml-4">
        <span className={`text-xs font-bold ${enabled ? "text-green-400" : "text-muted-foreground"}`}>
  ...
  return (
    <Card>
      ...
      <CardContent className="space-y-2">
        {LEAD_GEN_LOG.map((entry, i) => (
          <div key={i} className="flex items-center gap-3 p-3 rounded-lg bg-muted/30 border border-border hover:bg-accent/40 transition-colors">
            <div className={`w-2 h-2 rounded-full flex-shrink-0 ${i === 0 ? "bg-red-500 animate-pulse" : "bg-muted-foreground/40"}`} />
            <div className="flex-1 min-w-0">
              <p className="text-xs font-semibold text-foreground truncate">
                {i === 0 ? "Last storm event: " : ""}{entry.date} — {entry.event}
              </p>
            </div>
            <div className="flex items-center gap-3 flex-shrink-0 text-xs">
              <span className="text-amber-400 font-bold">{entry.created} created</span>
              <span className="text-green-400">{entry.dispatched} dispatched</span>
              {entry.pending > 0
                ? <span className="text-orange-400">{entry.pending} pending</span>
                : <span className="text-muted-foreground/60">0 pending</span>}
            </div>
          </div>
        ))}
      </CardContent>
...
function StormHistoryChart() {
  return (
    <Card>
      <CardHeader className="pb-3">
        <CardTitle className="text-base flex items-center gap-2">
          <BarChart3 className="w-4 h-4 text-orange-400" />
          Storm Events & Leads — Last 6 Months
        </CardTitle>
      </CardHeader>
      <CardContent>
        <ResponsiveContainer width="100%" height={220}>
          <BarChart data={STORM_HISTORY_DATA} margin={{ top: 4, right: 4, left: -20, bottom: 0 }}>
            ...
            <Bar yAxisId="left" dataKey="leads" fill="#f97316" radius={[4,4,0,0]} name="Leads" />
            <Bar yAxisId="right" dataKey="events" fill="#3b82f6" radius={[4,4,0,0]} name="Events" />
          </BarChart>
        </ResponsiveContainer>
        <div className="flex items-center gap-6 text-xs text-muted-foreground mt-2 justify-center">
          <span className="flex items-center gap-1.5"><span className="w-3 h-2 rounded-sm bg-orange-500 inline-block" />Leads generated</span>
          <span className="flex items-center gap-1.5"><span className="w-3 h-2 rounded-sm bg-blue-500 inline-block" />Storm events</span>
        </div>
      </CardContent>
    </Card>
  );
...
export default function StormDashboard() {
  ...
  const affectedZipCount = lastRunResult
    ...
    : (events ?? []).reduce((acc: number, e: any) => {
        const zoneList = (() => { try { return JSON.parse(e.affectedZones ?? "[]"); } catch { return []; } })();
        return acc + zoneList.length;
      }, 0);
    ...
    <AdminLayout>
      <div className="p-6 space-y-6">
        ...
        <AutoDispatchToggle
          ...
          onToggle={() => {
            setAutoDispatch(v => !v);
            toast.success(`Auto-dispatch ${autoDispatch ? "disabled" : "enabled"}`);
          }}
        />
            ...
            <div>
              <div className="flex items-center justify-between mb-3">
                <h2 className="text-sm font-semibold text-foreground flex items-center gap-2">
                  ...
                  {TOMORROW_API_KEY
                    ? <span className="text-xs text-teal-400 font-normal">· Tomorrow.io live</span>
                    : <span className="text-xs text-gray-500 font-normal">· Demo data</span>}
                </h2>
              </div>
                  ...
                  {eventsLoading ? (
                    ...
                    >
                      ...
                      <div className="flex items-center gap-3 mt-1">
                        <span className="text-xs text-muted-foreground flex items-center gap-1"><Zap className="w-3 h-3" />{evt.leadsGenerated} leads</span>
                        <span className="text-xs text-muted-foreground flex items-center gap-1"><MapPin className="w-3 h-3" />{evt.propertiesAffected} properties</span>
                        <span className="text-xs text-muted-foreground ml-auto">{new Date(evt.createdAt).toLocaleDateString()}</span>
                      </div>
                    </div>
                  ))}
                </CardContent>
        ...
        {activeTab === "analytics" && (
          <div className="space-y-6">
            <div className="grid grid-cols-3 gap-4">
              {[
                { label: "Avg Response Time",  value: `${(RESPONSE_ANALYTICS.reduce((a, s) => a + s.avgResponseMin, 0) / RESPONSE_ANALYTICS.length).toFixed(1)}m`, icon: Clock,       color: "text-green-400", sub: "median across last 5 storms"   },
                { label: "Acceptance Rate",    value: `${Math.round((RESPONSE_ANALYTICS.reduce((a, s) => a + s.accepted, 0) / RESPONSE_ANALYTICS.reduce((a, s) => a + s.leadsGen, 0)) * 100)}%`,                icon: TrendingUp, color: "text-teal-400",  sub: "leads accepted vs passed"      },
                { label: "Total Storm Revenue",value: "$4.7M",                                                                                                                            icon: ArrowUpRight,color: "text-amber-400",sub: "estimated from last 5 events"   },
              ].map(s => (
                <Card key={s.label}>
                  <CardContent className="p-4">
                    <div className="flex items-center gap-2 mb-1">
                      ...
                      return (
                        <div key={i} className="flex items-center gap-3">
                          ...
                          <div className="flex-1 bg-muted/40 rounded-full h-2">
                            <div className="bg-gradient-to-r from-orange-500 to-amber-400 h-2 rounded-full transition-all duration-700" style={{ width: `${pct}%` }} />
                          </div>
