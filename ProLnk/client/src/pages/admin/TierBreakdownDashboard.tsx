...
const PERFORMANCE_ROWS: TierPerformanceRow[] = [
  ...
  { tier: "L3", partners: 312, avgPPS: "4.72", avgJobsMo: 9.8, avgCommMo: "$610", retention: "94.1%", color: "text-teal-700" },
  { tier: "L4", partners: 1294, avgPPS: "4.60", avgJobsMo: 7.1, avgCommMo: "$420", retention: "91.3%", color: "text-gray-600" },
];

const TRANSITIONS: TierTransition[] = [
  { name: "Marcus T.", from: "L4", to: "L3", date: "May 14", fromColor: "bg-purple-500/20 text-gray-600", toColor: "bg-teal-500/20 text-teal-700" },
  { name: "Sarah K.", from: "L4", to: "L3", date: "May 13", fromColor: "bg-purple-500/20 text-gray-600", toColor: "bg-teal-500/20 text-teal-700" },
  { name: "James W.", from: "L3", to: "Founding", date: "May 12", fromColor: "bg-teal-500/20 text-teal-700", toColor: "bg-blue-500/20 text-teal-700" },
  { name: "Elena R.", from: "L4", to: "L3", date: "May 11", fromColor: "bg-purple-500/20 text-gray-600", toColor: "bg-teal-500/20 text-teal-700" },
  { name: "Carlos M.", from: "L4", to: "L3", date: "May 10", fromColor: "bg-purple-500/20 text-gray-600", toColor: "bg-teal-500/20 text-teal-700" },
  { name: "Patel D.", from: "L3", to: "Founding", date: "May 9", fromColor: "bg-teal-500/20 text-teal-700", toColor: "bg-blue-500/20 text-teal-700" },
  { name: "Kim J.", from: "L4", to: "L3", date: "May 8", fromColor: "bg-purple-500/20 text-gray-600", toColor: "bg-teal-500/20 text-teal-700" },
  { name: "Rivera A.", from: "L4", to: "L3", date: "May 8", fromColor: "bg-purple-500/20 text-gray-600", toColor: "bg-teal-500/20 text-teal-700" },
  { name: "Thompson B.", from: "L4", to: "L3", date: "May 7", fromColor: "bg-purple-500/20 text-gray-600", toColor: "bg-teal-500/20 text-teal-700" },
  { name: "Hassan F.", from: "L3", to: "Founding", date: "May 6", fromColor: "bg-teal-500/20 text-teal-700", toColor: "bg-blue-500/20 text-teal-700" },
  { name: "Okonkwo N.", from: "L4", to: "L3", date: "May 5", fromColor: "bg-purple-500/20 text-gray-600", toColor: "bg-teal-500/20 text-teal-700" },
  { name: "Larson P.", from: "L4", to: "L3", date: "May 4", fromColor: "bg-purple-500/20 text-gray-600", toColor: "bg-teal-500/20 text-teal-700" },
];
      ...
      <div className="max-w-6xl mx-auto px-4 py-10 space-y-8">
        ...
        <div className="flex items-start justify-between gap-4 flex-wrap">
          <div>
            ...
            <h1 className="text-3xl font-bold text-gray-900">Tier Breakdown</h1>
            <p className="text-gray-500 mt-1">Charter / Founding / L3 / L4 distribution</p>
          </div>
          <button className="flex items-center gap-2 bg-teal-500 hover:bg-teal-400 text-gray-900 font-semibold px-5 py-2.5 rounded-xl transition-colors text-sm">
            Recruit to L4
        ...
        <div className="grid grid-cols-3 gap-4">
          {[
            { label: "Total Active Partners", value: totalPartners.toLocaleString(), icon: Users, color: "text-teal-700" },
            ...
            const Icon = s.icon;
            return (
              <div key={s.label} className="rounded-xl bg-[#111D35] border border-gray-200/50 p-5 text-center">
        ...
        <div className="p-4 rounded-xl bg-amber-900/20 border border-amber-700/30 flex items-center gap-3">
          ...
          <p className="text-sm text-amber-200">
            <strong className="text-gray-900">L4 has 306 open spots.</strong>{" "}
            Founding has 2 open spots.{" "}
            <strong className="text-amber-700">Charter is full.</strong>{" "}
              ...
              <div key={tier.id} className={`rounded-2xl border ${tier.border} ${tier.bg} p-6`}>
                <div className="flex items-center justify-between mb-4">
                  ...
                  <div className="text-right">
                    <p className="text-2xl font-bold text-gray-900">{tier.filled}/{tier.capacity}</p>
                    <p className="text-xs text-gray-500">{pct}% filled</p>
                  </div>
                </div>
        ...
        {/* Tabs */}
        <div className="flex gap-1 bg-[#111D35] p-1 rounded-xl border border-gray-200/50 w-fit">
        ...
        {/* Tab Content */}
        {activeTab === "overview" && (
          <div className="rounded-2xl bg-[#111D35] border border-gray-200/50 p-6">
                ...
                <thead>
                  <tr className="text-left border-b border-gray-200">
                    {["Tier", "Partners", "Avg PPS", "Avg Jobs/mo", "Avg Comm/mo", "Retention"].map((h) => (
                      <th key={h} className="pb-3 pr-6 text-gray-500 font-medium whitespace-nowrap">{h}</th>
                    ))}
                  </tr>
                </thead>
                <tbody>
                  {PERFORMANCE_ROWS.map((row) => (
                    <tr key={row.tier} className="border-b border-gray-200/50 hover:bg-white/20 transition-colors">
                      <td className={`py-3 pr-6 font-semibold ${row.color}`}>{row.tier}</td>
                      <td className="py-3 pr-6 text-gray-800">{row.partners.toLocaleString()}</td>
                      <td className="py-3 pr-6 text-gray-800">{row.avgPPS}</td>
                      <td className="py-3 pr-6 text-gray-800">{row.avgJobsMo}</td>
        ...
        {activeTab === "transitions" && (
          <div className="rounded-2xl bg-[#111D35] border border-gray-200/50 p-6">
            <h3 className="text-lg font-semibold text-gray-900 mb-1">Tier Transitions — Last 30 Days</h3>
            <p className="text-sm text-gray-500 mb-4">12 partners upgraded in the last 30 days</p>
            <div className="space-y-2">
              {TRANSITIONS.map((t, i) => (
                <div key={i} className="flex items-center gap-4 py-2.5 border-b border-gray-200/50">
                  ...
                  <span className="text-xs text-gray-500 w-14 text-right">{t.date}</span>
                </div>
              ))}
            </div>
          </div>
