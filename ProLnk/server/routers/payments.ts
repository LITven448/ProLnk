      ...
      try {
        ...
        if (partner?.email) {
          await sendPayoutConfirmation({
            ...
            dashboardUrl: `${process.env.VITE_OAUTH_PORTAL_URL || process.env.APP_BASE_URL || 'https://prolnk.xyz'}/dashboard/earnings`,
          });
        }
      } catch (emailErr) {
      ...
      try {
        ...
        if (email) {
          await sendPayoutConfirmation({
            ...
            method: 'Stripe Connect',
            periodLabel: new Date().toLocaleDateString('en-US', { month: 'long', year: 'numeric' }),
            dashboardUrl: `${process.env.VITE_OAUTH_PORTAL_URL || process.env.APP_BASE_URL || 'https://prolnk.xyz'}/dashboard/payout-history`,
          });
        }
      } catch {}
