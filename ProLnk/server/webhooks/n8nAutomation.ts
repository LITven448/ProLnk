...
export const automations = {
  partnerWaitlistJoined: (data: { email: string; tier: string; position: number; referralCode: string }) =>
    triggerN8n("partner-waitlist-joined", data),

  homeownerWaitlistJoined: (data: { email: string; city: string; serviceNeeded: string }) =>
  ...
  jobCompleted: (data: { jobId: string; proEmail: string; jobValue: number; address: string }) =>
    triggerN8n("job-completed", data),

  stormDetected: (data: { state: string; zipCodes: string[]; severity: string }) =>
    triggerN8n("storm-detected", data),

  weeklyDigest: (data: { week: string }) =>
    triggerN8n("weekly-digest", data),

  partnerInactive: (data: { email: string; name: string; daysSinceLastJob: number }) =>
