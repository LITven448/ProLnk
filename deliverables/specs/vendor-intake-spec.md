...
- **R-1.3.9.1** Access model (RETIRED per founder decision — vendors are never charged per work order; licensee-locked vendors are free, marketplace access requires a subscription): (a) **Subscription** `$99/mo (matches existing Core tier)`/mo, unlimited operator work orders; (b) **Per-work-order** `$0 — vendors are not charged per work order` deducted/netted per completed work order (audit 5.4). Radio, required.
