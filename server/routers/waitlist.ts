...
export const waitlistRouter = router({
      ...
      return await logger.track("waitlist:joinProWaitlist", async () => {
        ...
        if (input.referredBy) {
          const [refByCode] = await pool.query(
            "SELECT id FROM proWaitlist WHERE referralCode = ? LIMIT 1",
            [input.referredBy.toUpperCase()]
          );
          if ((refByCode as any[])[0]) {
            referrerId = (refByCode as any[])[0].id;
          } else {
            const [refBySlug] = await pool.query(
              "SELECT id FROM proWaitlist WHERE customSlug = ? LIMIT 1",
              [input.referredBy.toLowerCase()]
            );
            if ((refBySlug as any[])[0]) {
              referrerId = (refBySlug as any[])[0].id;
            }
          }
        }
  ...
  claimSlug: publicProcedure
    ...
    .mutation(async ({ input }) => {
      ...
      if (!SLUG_REGEX.test(slug)) {
        throw new TRPCError({
          code: "BAD_REQUEST",
          message: "Slug must be 3-30 characters, lowercase alphanumeric and hyphens only, no leading/trailing hyphens.",
        });
      }
      ...
      const [userRows] = await pool.query(
        "SELECT id, customSlug FROM proWaitlist WHERE email = ? LIMIT 1",
        [input.email]
      );
      ...
      try {
        await pool.query("ALTER TABLE proWaitlist ADD COLUMN customSlug VARCHAR(64) UNIQUE", []);
      } catch {
      ...
      return {
        success: true as const,
        slug,
        shareUrl: `https://prolnk.io/join/${slug}`,
      };
    }),
  ...
  resolveSlug: publicProcedure
    ...
    .query(async ({ input }) => {
      ...
      const [rows] = await pool.query(
        "SELECT firstName, businessType, referralCode FROM proWaitlist WHERE customSlug = ? LIMIT 1",
